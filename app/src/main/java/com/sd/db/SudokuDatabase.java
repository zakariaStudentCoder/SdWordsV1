...
public class SudokuDatabase {
	...
	public WordWrapper getWord(String word)
	{
		SQLiteQueryBuilder qb = new SQLiteQueryBuilder();

		qb.setTables(WORD_TABLE_NAME);
		qb.appendWhere(WordsColumns.WORD + "=" + word);
		...
		try {
			...
			if (c.moveToFirst()) {
				long id = c.getLong(c.getColumnIndex(WordsColumns._ID));
				String w = c.getString(c.getColumnIndex(WordsColumns.WORD));
				String desc = c.getString(c.getColumnIndex(WordsColumns.DESCRIPTION));

				WordWrapper wW = new WordWrapper(id, w, desc);
	...
	public WordWrapper getWord(long wId)
	{
		SQLiteQueryBuilder qb = new SQLiteQueryBuilder();

		qb.setTables(WORD_TABLE_NAME);
		qb.appendWhere(WordsColumns._ID + "=" + wId);
